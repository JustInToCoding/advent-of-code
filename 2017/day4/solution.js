...
const validPassphrases = (passphrases, isPassphraseValid) => {
    return passphrases.split('\n').reduce((acc, curr) => {
        const passphrase = curr.split(' ');
        let isCurrentPassphraseValid = true;
        const isWordValid = isPassphraseValid(passphrase);
        while(word = passphrase.shift()){
            if(isWordValid(word)) {
                isCurrentPassphraseValid = false;
            }
        }
        return acc + isCurrentPassphraseValid;
    }, 0);
};

const sortStringAlphabetically = (value) => value.split('').sort().join('');
const isPassphraseValidPart1 = (passphrase) => (word) => passphrase.indexOf(word) > -1;
const isPassphraseValidPart2 = (passphrase) => (word) => passphrase.map(sortStringAlphabetically).indexOf(sortStringAlphabetically(word)) > -1;

console.log(validPassphrases(input, isPassphraseValidPart1));
console.log(validPassphrases(input, isPassphraseValidPart2));
